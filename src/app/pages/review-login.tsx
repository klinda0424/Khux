...
export function ReviewLogin() {
  ...
  useEffect(() => {
    if (!loading && user) {
      const redirect = sessionStorage.getItem("khux_redirect_after_login") || "/members";
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-card border border-border rounded-xl p-8 shadow-lg">
          <a
            ...
            className="w-full flex items-center justify-center gap-3 py-3 px-4 bg-[#5865F2] text-white rounded-lg font-medium hover:bg-[#4752C4] transition-colors"
          >
          ...
          <div className="mt-6 pt-6 border-t border-border space-y-2">
            <p className="text-sm text-muted-foreground text-center">
              KHUX 디스코드 서버에 가입된 계정으로 로그인해주세요.
            </p>
            <p className="text-xs text-muted-foreground text-center">
              디스코드 역할(Role)을 기반으로 소속 팀이 자동 인식됩니다.
            </p>
          </div>
        </div>
      </div>
