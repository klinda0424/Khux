...
const HOURS = Array.from({ length: 15 }, (_, i) => i + 8); // 8시~22시
...
function formatDateHeader(dateStr: string) {
  ...
  const label = dateStr === today ? "오늘" : `${DAY_KO[d.getDay()]}`;
  return { label, date: dateStr.slice(5).replace("-", "/") };
}
...
export function WhenToMeetSchedule() {
  ...
  function handleMouseDown(date: string, hour: number) {
    if (!isOwn) return;
    const set = availability[date] ?? new Set();
    const mode = set.has(hour) ? "remove" : "add";
    setDragMode(mode);
    setIsDragging(true);
      ...
      <div className="max-w-5xl mx-auto pt-14 pb-24">
        <div className="mb-6 px-2">
          ...
          {isOwn ? (
            <p className="text-sm text-muted-foreground mt-1">
              가능한 시간대를 클릭하거나 드래그해서 선택하세요. (초록색 = 가능)
            </p>
          ) : (
            <p className="text-sm text-muted-foreground mt-1">읽기 전용 — 본인 일정만 수정할 수 있어요.</p>
          )}
        </div>

        {error && (
          <div className="mb-4 mx-2 p-3 bg-destructive/10 border border-destructive/20 rounded-lg text-sm text-destructive">
            {error}
          </div>
        )}
        ...
        <div className="flex items-center gap-4 mt-4 px-2 text-xs text-muted-foreground">
          <div className="flex items-center gap-1.5">
            <div className="w-4 h-4 rounded bg-green-500/70" /> 가능
          </div>
          <div className="flex items-center gap-1.5">
            <div className="w-4 h-4 rounded bg-muted/30 border border-border/50" /> 불가능
          </div>
        </div>
      </div>
